...
def trainDigits(dataArr,labelArr,kTup=('rbf', 10)):
    ...
    for i in range(m):
        ...
        print('the realizes value is: %d', labelArr[i])
...
#把数据依靠','区分，并分别读入\n",
trainning_list = trainning_data_list[1:10001]
dataArr = []
labelArr =[]
...
print(labelArr)
######数据训练#######
trainDigits(dataArr,labelArr,kTup=('rbf', 10))#  K = exp(K/(-1*kTup[1]**2))#k = e^{k/(-1*kTup[1]**2)}  10为参数
